...
import com.Exchange.Exchange.api.bybit.KlineBybit_api;
...
@Service
public class KlineBybitConnect {
    @Getter
    private Map<Long, List<String>> bybitData = new ConcurrentHashMap<>();
    ...
    public void BybitDataAll(String symbol, List<Long> timePoints, String interval) {
        ...
        KlineBybit_api bybitClass = new KlineBybit_api();
            ...
            for (List<TimeRange> chunk : chunks) {
                List<CompletableFuture<Map<Long, List<String>>>> futures = chunk.stream()
                        .map(batch -> CompletableFuture.supplyAsync(() -> {
                            try {
                                return bybitClass.klineBybit(httpClient, symbol, batch.start, batch.end, interval);
                            } catch (Exception e) {
                ...
                CompletableFuture.allOf(futures.toArray(new CompletableFuture[0]))
                        .thenApply(v -> futures.stream()
                                ...
                                .collect(Collectors.toList()))
                        .thenAccept(results -> {
                            results.forEach(bybitData::putAll);
                        }).join();
