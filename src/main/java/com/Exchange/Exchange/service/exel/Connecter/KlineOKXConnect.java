...
import com.Exchange.Exchange.api.OKX.KlineOKX_api;
...
@Service
public class KlineOKXConnect {
    @Getter
    private Map<Long, List<String>> OKXData = new ConcurrentHashMap<>();
    ...
    public void OKXDataAll(String symbol, List<Long> timePoints, String interval) {
        ...
        KlineOKX_api OKXClass = new KlineOKX_api();
            ...
            for (List<TimeRange> chunk : chunks) {
                List<CompletableFuture<Map<Long, List<String>>>> futures = chunk.stream()
                        .map(batch -> CompletableFuture.supplyAsync(() -> {
                            try {
                                return OKXClass.klineOKX(httpClient, symbol, batch.start, batch.end, interval);
                            } catch (Exception e) {
                ...
                CompletableFuture.allOf(futures.toArray(new CompletableFuture[0]))
                        .thenApply(v -> futures.stream()
                                ...
                                .collect(Collectors.toList()))
                        .thenAccept(results -> {
                            results.forEach(OKXData::putAll);
                        }).join();
