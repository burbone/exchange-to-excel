...
import com.Exchange.Exchange.api.bitget.KlineBitget_api;
...
@Service
public class KlineBitgetConnect {
    @Getter
    private Map<Long, List<String>> bitgetData = new ConcurrentHashMap<>();
    ...
    public void BitgetDataAll(String symbol, List<Long> timePoints, String interval) {
        ...
        KlineBitget_api bitgetClass = new KlineBitget_api();
            ...
            for (List<TimeRange> chunk : chunks) {
                List<CompletableFuture<Map<Long, List<String>>>> futures = chunk.stream()
                        .map(batch -> CompletableFuture.supplyAsync(() -> {
                            try {
                                return bitgetClass.klineBitget(httpClient, symbol, batch.start, batch.end, interval);
                            } catch (Exception e) {
                ...
                CompletableFuture.allOf(futures.toArray(new CompletableFuture[0]))
                        .thenApply(v -> futures.stream()
                                ...
                                .collect(Collectors.toList()))
                        .thenAccept(results -> {
                            results.forEach(bitgetData::putAll);
                        }).join();
